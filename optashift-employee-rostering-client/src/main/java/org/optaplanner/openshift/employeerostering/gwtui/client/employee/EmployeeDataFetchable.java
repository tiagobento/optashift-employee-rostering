    ...
    public void fetchData(Command after) {
        if (!busy) {
            ...
            if (null == last || !last.getTenantId().equals(tenantId)) {
                ...
                RosterRestServiceBuilder.getCurrentEmployeeRosterView(tenantId, new FailureShownRestCallback<EmployeeRosterView>() {
                    ...
                    public void onSuccess(EmployeeRosterView employeeRosterView) {
                        try {
                            ...
                            Map<Long, Map<Long, List<ShiftView>>> timeSlotIdToEmployeeIdToShiftViewListMap = employeeRosterView.getTimeSlotIdToEmployeeIdToShiftViewListMap();

                            Map<Long, Map<Long, EmployeeAvailabilityView>> timeSlotIdToEmployeeIdToAvailabilityViewMap = employeeRosterView.getTimeSlotIdToEmployeeIdToAvailabilityViewMap();
                            Map<Long, Spot> spotMap = employeeRosterView.getSpotList()
                                    .stream()
                            ...
                            for (TimeSlot timeslot : timeslots) {
                                for (Employee employee : employees) {
                                    if (null != timeSlotIdToEmployeeIdToShiftViewListMap.get(timeslot.getId()).get(employee.getId())) {
                                        timeSlotIdToEmployeeIdToShiftViewListMap.get(timeslot.getId())
                                                .get(employee.getId())
                                                .stream()
                                                .forEach((sv) -> {
                                                    ...
                                                    out.add(new EmployeeData(shift,
                                                            timeSlotIdToEmployeeIdToAvailabilityViewMap.getOrDefault(timeslot.getId(),
                                                                    Collections.emptyMap()).get(employee.getId())));
                                                });
                                    } else {
                ...
                RosterRestServiceBuilder.getEmployeeRosterViewFor(tenantId, calendar.getViewStartDate().toLocalDate().toString(),
                        calendar.getViewEndDate().toLocalDate().toString(),
                        calendar.getVisibleGroupSet().stream().map((g) -> g.getEmployee()).collect(Collectors.toList()),
                        new FailureShownRestCallback<EmployeeRosterView>() {
