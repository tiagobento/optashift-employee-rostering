...
public class SpotShiftEditForm implements IsElement {
    ...
    protected void initWidget() {
        ...
        EmployeeRestServiceBuilder.getEmployeeList(spot.getData().getShift().getTenantId(), new FailureShownRestCallback<List<Employee>>() {
            ...
            public void onSuccess(List<Employee> tenantEmployeeList) {
                ...
                if (!spot.getData().isLocked()) {
                    ...
                    assignedEmployee.setSelectedIndex(employeeList.indexOf(spot.getData().getAssignedEmployee()));
                }
    ...
    public void save(ClickEvent click) {
        if (isLocked.getValue()) {
            Employee employee = employeeList.stream()
                    .filter((e) -> e.getName().equals(assignedEmployee.getSelectedValue()))
                    .findFirst()
                    .get();
            ...
            popup.hide();
            ShiftRestServiceBuilder.updateShift(spot.getData().getShift().getTenantId(), shiftView, new FailureShownRestCallback<Void>() {
